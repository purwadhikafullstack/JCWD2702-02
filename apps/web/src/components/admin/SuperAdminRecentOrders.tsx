...
import { useGetRecentOrders } from "@/helpers/adminReport/hooks/useGetRecentOrders";
import { FaShoppingBag } from "react-icons/fa";
import { formatDistanceToNow } from "date-fns";

export default function SuperAdminRecentOrders() {
    const { dataRecentOrders } = useGetRecentOrders();

    if (!dataRecentOrders) return <div>Loading...</div>
    ...
    return (
        <div className="w-full col-span-1 relative h-[95vh] m-auto p-4 border rounded-lg bg-white overflow-y-auto">
            ...
            <ul>
                {dataRecentOrders.map((order: { id: number; totalOrderAmount: number; user: { fullname: string }; createdAt: Date }, index: number) => (
