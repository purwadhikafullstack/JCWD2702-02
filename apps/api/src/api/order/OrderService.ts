...
export class OrderService {
    ...
    async findNearestWarehouse(address: string): Promise<Warehouse | null> {
      const userLocation = { latitude: -6.200000, longitude: 106.816666 };
      ...
      for (const warehouse of warehouses) {
        const distance = this.calculateDistance(userLocation, { latitude: warehouse.latitude, longitude: warehouse.longitude });
