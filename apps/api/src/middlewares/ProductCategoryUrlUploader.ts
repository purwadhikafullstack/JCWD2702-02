...
export const productCategoryUrlUploader = (req: Request, res: Response, next: NextFunction) => {
    const upload = multerProductCategoryUrlUpload.fields([{ name: 'categoryurl', maxCount: 1 }]);

    upload(req, res, (err) => {
        try {
            if (err) throw new Error(err.message);
            if (req.files) {
                const uploadedFiles = Array.isArray(req.files) ? req.files : req.files['categoryurl'];
